...
const SellerPanel = () => {
    ...
    const fetchAllData = async () => {
        setLoading(true);
        try {
            const [sRes, pRes, oRes, tRes, slRes, puRes] = await Promise.all([
    ...
    const updateOrderStatus = async (orderId, newStatus, newDeliveryOption) => {
        try {
            await axios.put(`/api/orders/${orderId}/status`, { status: newStatus, deliveryOption: newDeliveryOption });
        ...
        <div className="min-h-screen bg-[#ffffff] flex flex-col md:flex-row no-copy w-full font-['Spartan']">
            ...
            <div className="w-full md:w-64 bg-[#041e42] text-white p-6 flex flex-col gap-5 md:min-h-screen ">
                <div className="flex items-center gap-2">
                    <div className="bg-orange-500 p-2.5 rounded-2xl shadow-lg shadow-orange-500/20"><StoreIcon size={24} /></div>
                    <div>
                        <h2 className="text-xl font-black tracking-tight">{currentUser?.shopName || 'SELLER SHOP'}</h2>
                        <p className="text-[10px] text-orange-400 font-bold uppercase tracking-widest">Seller Panel</p>
                    </div>
                </div>
                ...
                <nav className="flex flex-col gap-2">
                    {[
                        ...
                        { id: 'orders', icon: ShoppingCart, label: 'Customer Orders' },
                        { id: 'history', icon: FileSpreadsheet, label: 'History' },
                        { id: 'messages', icon: FileText, label: 'Messages' },
                    ].map(tab => (
                        <button
                            key={tab.id}
                            onClick={() => setActiveTab(tab.id)}
                            className={`flex items-center gap-4 px-5 py-4 rounded-xl font-small transition-all ${activeTab === tab.id ? 'bg-orange-500 text-white shadow-lg shadow-orange-700/20 scale-105' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
                        >
            ...
            <div className="flex-1 p-6 md:p-12 overflow-x-hidden">
                <header className="mb-12 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    ...
                    <div className="flex gap-3">
                        {activeTab === 'inventory' && (
                            <button onClick={() => { setIsProductModalOpen(true); setEditingItem(null); setProductForm({ name: "", sellingPrice: "", purchasePrice: "", description: "", category: "General", image: "", stock: 0, unit: "পিস", liveStatus: "live" }); }} className="bg-orange-500 text-white px-6 py-3.5 rounded-xl font-bold shadow-xl shadow-orange-900/10 flex items-center gap-2 hover:scale-105 transition-all">
                                <Plus size={20} /> Add Product
                            </button>
                        )}
                        {activeTab === 'cashbox' && (
                            <div className="flex gap-2">
                                <button onClick={() => setIsTransactionModalOpen(true)} className="bg-orange-500 text-white px-6 py-3.5 rounded-xl font-bold shadow-xl flex items-center gap-2 hover:scale-105 transition-all">
                                    <Plus size={20} /> Add Transaction
                                </button>
                                <button onClick={() => { setTransactionForm({ type: "Cash Out", amount: "", description: "Cash out request to Admin" }); setIsTransactionModalOpen(true); }} className="bg-purple-600 text-white px-6 py-3.5 rounded-xl font-bold shadow-xl flex items-center gap-2 hover:scale-105 transition-all">
                                    <Banknote size={20} /> Request Cash Out (2% Fee)
                                </button>
                            </div>
                        )}
                        {activeTab === 'sales' && (
                ...
                {loading ? (
                    ...
                    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
                                    ...
                                    <tbody className="divide-y divide-gray-50">
                                        {orders.map(o => (
                                            <tr key={o._id} className="hover:bg-gray-50/50 transition-colors">
                                                ...
                                                <td className="px-8 py-6">
                                                    <select value={o.status} onChange={(e) => updateOrderStatus(o._id, e.target.value, o.deliveryOption)} className="text-[10px] font-black uppercase bg-orange-50 text-orange-600 rounded-xl px-3 py-1.5 border border-orange-100 outline-none mb-2 w-full">
                                                        <option value="Pending">Pending</option>
                                                        <option value="Processing">Processing (Accept)</option>
                                                        <option value="Delivered">Delivered (Complete)</option>
                                                        <option value="Cancelled">Cancelled</option>
                                                    </select>
                                                </td>
                                                <td className="px-8 py-6 text-center">
                                                    <div className="text-[10px] font-bold text-gray-400 mb-2">{o.shippingAddress?.slice(0, 20)}...</div>
                                                    <div className="flex flex-col gap-1 items-center">
                                                        <div className={`text-[8px] font-black px-2 py-0.5 rounded-full inline-block ${o.isPaid ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{o.isPaid ? 'PAYMENT RECEIVED' : 'UNPAID'}</div>
                                                        <select value={o.deliveryOption || 'Standard'} onChange={(e) => updateOrderStatus(o._id, o.status, e.target.value)} className="text-[8px] font-black uppercase bg-gray-50 text-gray-700 rounded-full px-2 py-0.5 mt-1 border border-gray-200 outline-none max-w-[100px]">
                                                ...
                                                <td className="px-8 py-6 text-right">
                                                    <button className="bg-gray-50 p-2.5 rounded-xl text-gray-400 hover:text-orange-500 transition-colors"><FileText size={18} /></button>
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
                        ...
                        {activeTab === 'messages' && (
                            <div className="bg-white rounded-[1rem] shadow-sm border border-orange-500 overflow-hidden p-8 text-center">
                                <h3 className="text-2xl font-black mb-4">Messages</h3>
                                <p className="text-gray-500 mb-8">Direct messaging with Users and Admin is coming soon!</p>
                                <div className="text-6xl mb-4">💬</div>
                            </div>
                        )}

                        {activeTab === 'history' && (
                            <div className="bg-white rounded-[1rem] shadow-sm border border-orange-500 overflow-hidden p-8">
                                <h3 className="text-2xl font-black mb-4">Export History</h3>
                                <p className="text-gray-500 mb-8">Download your data in Excel (.xlsx) format.</p>
                                <button onClick={() => exportToExcel(orders, "Seller_Orders_History")} className="bg-green-600 text-white font-bold py-3 px-6 rounded-xl flex items-center gap-2 hover:bg-green-700 mx-auto">
                                    <Download size={20} /> Download Orders Excel
                                </button>
                            </div>
                        )}
                    </motion.div>
