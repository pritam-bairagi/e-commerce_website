...
import { Loader, Lock, Mail, User, Phone, Truck } from "lucide-react";
...
const CourierRegister = () => {
    ...
    const handleSignUp = async (e) => {
        ...
        setIsLoading(true);
        try {
            const data = await register({ name, email, password, phoneNumber, role: 'courier' });
        ...
        >
            <div className='p-8'>
                ...
                <form onSubmit={handleSignUp} className="space-y-4">
                    ...
                    <motion.button
                        className='mt-5 w-full py-3 px-4 bg-gradient-to-r from-orange-500 to-red-600 text-white font-bold rounded-lg shadow-lg hover:from-orange-600 hover:to-red-700 focus:outline-none focus:ring-2 focus:ring-orange-500 transition duration-200'
                        whileHover={{ scale: 1.02 }}
                        whileTap={{ scale: 0.98 }}
                        type='submit'
                        disabled={isLoading}
                    >
                        {isLoading ? <Loader className=' animate-spin mx-auto' size={24} /> : "Register as Courier"}
                    </motion.button>
                </form>
            </div>
